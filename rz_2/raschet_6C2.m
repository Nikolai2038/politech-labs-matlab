...
maxQ
m = 7; %Кол-во точек аппроксимации. Возможные значения:7-301

for s = 1:m
    b(s) = round((s - 1) * ((2 * d / 0.001) / (m - 1)) + 1);
    R1(s) = R(b(s));
end
...
%НАХОЖДЕНИЕ УТОЧНЕННОГО ЗНАЧЕНИЯ ПОЛОЖЕНИЯ МАКСИМУМА ФКК
%фКК - функция коэффициентов корреляции
err = 0;
cmax = max(yint); %нахождение значения максимума функции коэф. корр.

for i = 1:round((m - 1) / 0.1 + 1)

    if (yint(i) == cmax)
        err_int = -0.1 + i * (0.2 / (10 * m));
    end
