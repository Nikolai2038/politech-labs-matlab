...
classdef application < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function GenerateButtonPushed(app, event)
            ...
            for i=1:N
                H(i)=1/(sqrt(1+(i/NC).^4)); %передаточная функция фильтра Баттерворта 4-го порядка
            end
            i=1:200;
            %%%figure
            %plot(i,abs(H(1:200)));
            ...
            Z=ifft(XX1.*H);%свертка частотного спектра зашумленного сигнала с частотной хар-кой фильтра
            XX2=fft(s,N);%частотный спектр сигнала без шума
            Z2=ifft(XX2.*H);%свертка сигнала с частотной хар-кой фильтра
            DZ(i)=(2*real(Z(i))-2*real(Z2(i)))*100./(max(s)-min(s));%вычисление случайной составляющей погрешности (после корректировки)
            DZ1(i)=(2*real(Z(i))-s(i))*100./(max(s)-min(s)); %вычисление полной погрешности(после корректировки)
            ...
            i=1:N;
            plot(app.UIAxesSignals_3_Butterworth, i,2*real(Z(1:N)),'m'); %вывод отфильтрованного сигнала
            ...
            %%%title('Случайная погрешность');
            %%%i=1:N;
            %%%figure
            %%%plot(i,real(DZ1(1:N))); %вывод отфильтрованного сигнала
            %%%title('Полная погрешность');
            % ----------------------------------------
            ...
            i=1:N;
            plot(app.UIAxesSignals_3_Chebyshev, i,2*real(Z(1:N)),'m'); %вывод отфильтрованного сигнала
            ...
            e=0.1;%параметр фильтра Чебышёва %вместо 0.001
            %%%figure 
            %%%plot(x);
            %%%axis tight; %диапазон X и Y по осям точно соответствует Xmax и Kmax
            %%%title('Зашумленный сигнал до фильтра');
            ...
            for i=1:N;    
                Tn=2*(NC/i)^2-1;%значение полинома Чебышёва 1 рода 2-го порядка  
                %H(i)=1/sqrt(1+e^2*Tn^2);%частотная характеристика фильтра Чебышёва
                H(i)=sqrt((e^2*Tn^2)/((1+e^2*Tn^2)));%частотная характеристика фильтра Чебышёва инверсный
            end
            h=ifft(H);
            %%%i=1:N;
            %%%plot(i(1:20),abs(h(1:20))); %Импульсная характеристика фильтра
            ...
            %масштабе по Х
            %%%grid on;
            %%%title('Частотная хар-ка цифрового фильтра инверсного Чебышёва');
            ...
            i=1:N;
            plot(app.UIAxesSignals_3_ChebyshevInverse, i,2*real(Z(1:N)),'r-'); %вывод отфильтрованного сигнала
            %представление графика линией красного цвета, отображение сетки
            %%%title('Сигнал до и после фильтра');%подпись названия графика
            %%%hold off;
            ...
            % ----------------------------------------
            NC=64;% полоса пропускания фильтра по уровню 0,7 амплитуды 
            % выражена в количестве отчетов спектра БПФ, пропускаемых фильтром
            % остальные отсчеты (в частотном спектре!) будут ослабляться по амплитуде
            ...
            %%%i=1:N;
            %%%figure
            %%%plot(i(1:200),2*abs(Y(1:200)));
            %semilogy(i(1:200),2*abs(Y(1:200)));%то же, что и plot, но в логарифмическом
            %масштабе по Х 
            %%%title('Частотный спектр сигнала с шумом');
                   
            for i=1:N    
                H(i)=1/((1+j*i/(NC))); %передаточная функция цифрового фильтра НЧ 1-го порядка
                %в частотной области
            end
            %%%h=ifft(H);
            %%%i=1:N;
            ...
            %масштабе по Х
            %%%grid on;
            %%%title('Частотная хар-ка цифрового НЧ-фильтра');
            ...
            %%%axis tight; %диапазон X и Y по осям точно соответствует Xmax и Ymax
            %%%hold on; % "удержание" окна вывода для вывода следующего графика
            plot(app.UIAxesSignals_3_LowPass, i,2*real(Z(1:N)),'r-'); %вывод отфильтрованного сигнала
            %представление графика линией красного цвета, отображение сетки
            %%%title('Сигнал до и после фильтра');%подпись названия графика
            %%%hold off;
